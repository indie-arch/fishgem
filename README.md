...
Six fish archetypes exercise the rules in `fishing.md`: common, fast, strong, tiny, large, and rare. Three separate upgrade tracks each have three levels:
...
After a successful hit, the fish dashes to a random destination over 0.18 seconds. Track it and reacquire the target; clicks during that brief dash are ignored. Fish names, prices, encounter frequency and difficulty are provisional values for playtesting.
